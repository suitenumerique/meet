...
function startPolling(csrfToken, { onSuccess, onTimeout, onError }) {
  ...
  const pollInterval = setInterval(() => {
    ...
    fetch(`${BASE_URL}/api/v1.0/addons/sessions/poll/`, {
      method: "POST",
      credentials: "include",
      headers: {
...
function openTransitDialog(transitToken, { onCancel, onError }) {
  ...
  console.log("Opening transit dialog...");

  const meetUrl = `${BASE_URL}/addons/outlook/transit.html`;
  ...
  Office.context.ui.displayDialogAsync(
    ...
    (asyncResult) => {
      ...
      dialog.addEventHandler(Office.EventType.DialogEventReceived, (arg) => {
        ...
        console.log("DialogEventReceived");
      ...
      dialog.addEventHandler(Office.EventType.DialogMessageReceived, (arg) => {
        ...
        // otherwise treat as cancel signal
        onCancel?.();
        dialog.close();
      });
...
function connect() {
  ...
  fetch(`${BASE_URL}/api/v1.0/addons/sessions/init/`, {
    method: "POST",
    ...
    .then((data) => {
      ...
      const transitToken = data.transit_token;

      setStatus("En attente d'authentification...");

      const pollInterval = startPolling(csrfToken, {
        onSuccess: (sessionData) => {
      ...
      openTransitDialog(transitToken, {
