...
| ALLOW_PUBLIC_ROOMS                              | Allow rooms open to anyone with the link. A change takes effect on the next restart, which is when the backend refuses that level wherever it is set, answers 404 for unregistered rooms, and moves the public rooms and user defaults it already holds to trusted, for good | true                                                                                                                                                          |
