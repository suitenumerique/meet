...
| RESOURCE_DEFAULT_ACCESS_LEVEL                   | Default resource access level for rooms. Cannot be public while ALLOW_PUBLIC_ROOMS is off                                                                    | public                                                                                                                                                        |
| ALLOW_PUBLIC_ROOMS                              | Allow rooms open to anyone with the link. Takes effect on the next restart: the room APIs then refuse the level, unregistered rooms answer 404, and a room stored public runs as trusted. Run manage.py forbid_public_rooms to move the stored rooms and user defaults to trusted for good, with --dry-run listing them first. Without it, a room keeps its stored level and comes back if the setting is turned on again; a write of the level stores it | true                                                                                                                                                          |
