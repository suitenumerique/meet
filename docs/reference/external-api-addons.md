...
| `ADDONS_TOKEN_SCOPE` | Yes | - | Scope in issued tokens (e.g. `external_api`) |
| `ADDONS_SESSION_TTL` | No | `300` | Session lifetime in seconds |
| `ADDONS_TRANSIT_TOKEN_TTL` | No | `60` | Transit token lifetime in seconds |
| `ADDONS_TOKEN_TTL` | No | `3600` | Access token lifetime in seconds |
| `ADDONS_TOKEN_ISSUER` | No | `meet-addons` | JWT issuer claim |
| `ADDONS_TOKEN_AUDIENCE` | No | `meet-external-api` | JWT audience claim |
